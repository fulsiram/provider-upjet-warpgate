...
PROJECT_NAME ?= provider-upjet-warpgate
PROJECT_REPO ?= github.com/fulsiram/$(PROJECT_NAME)
...
export TERRAFORM_PROVIDER_SOURCE ?= warp-tech/warpgate
export TERRAFORM_PROVIDER_REPO ?= https://github.com/warp-tech/terraform-provider-warpgate
export TERRAFORM_PROVIDER_VERSION ?= 1.1.6
export TERRAFORM_PROVIDER_DOWNLOAD_NAME ?= terraform-provider-warpgate
export TERRAFORM_PROVIDER_DOWNLOAD_URL_PREFIX ?= https://github.com/warp-tech/terraform-provider-warpgate/releases/download/v$(TERRAFORM_PROVIDER_VERSION)
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-warpgate_v$(TERRAFORM_PROVIDER_VERSION)
...
REGISTRY_ORGS ?= ghcr.io/fulsiram
IMAGES = $(PROJECT_NAME)
...
XPKG_REG_ORGS ?= ghcr.io/fulsiram
# NOTE(hasheddan): skip promoting on xpkg.crossplane.io as channel tags are
# inferred.
XPKG_REG_ORGS_NO_PROMOTE ?= ghcr.io/fulsiram
XPKGS = $(PROJECT_NAME)
...
xpkg.build.provider-upjet-warpgate: do.build.images
