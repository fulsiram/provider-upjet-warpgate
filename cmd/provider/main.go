...
import (
	...
	apisCluster "github.com/fulsiram/provider-upjet-warpgate/apis/cluster"
	apisNamespaced "github.com/fulsiram/provider-upjet-warpgate/apis/namespaced"
	"github.com/fulsiram/provider-upjet-warpgate/config"
	"github.com/fulsiram/provider-upjet-warpgate/internal/clients"
	controllerCluster "github.com/fulsiram/provider-upjet-warpgate/internal/controller/cluster"
	controllerNamespaced "github.com/fulsiram/provider-upjet-warpgate/internal/controller/namespaced"
	"github.com/fulsiram/provider-upjet-warpgate/internal/features"
	"github.com/fulsiram/provider-upjet-warpgate/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Warpgate").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-upjet-warpgate"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-upjet-warpgate",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Warpgate APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Warpgate APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-upjet-warpgate:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Warpgate controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Warpgate controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Warpgate controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Warpgate controllers")
	}
